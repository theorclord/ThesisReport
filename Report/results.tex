...
When the players reach an intermediary island they are informed that an artifact has been collected by a panel appearing after solving the event.
...
The \textit{Scope} of this prototype covers all the principles previously mentioned all along this paper. It is the first time the \textit{faction} system and the special events are introduced in the game, and are not tested separately. This is tested along with the \textit{Specialization} system. Both mechanics should support each other, and the players should make informed decisions when making their choices. The specialization chosen by one player must give her the authority to perform action, and no player should be the \textit{leader}.

About the game itself, a progression in the difficulty must be visible when the players reach the intermediary islands.  The difficulty must increase, and the decisions should be taken more carefully. Also, the players must realize that their choices have influenced the game and the generation of content.
