...
\subsection{Game setting and story}
...
\end{itemize}
It was not the first iteration of the implementation of a digital device in the flow of the game: an iteration of the procedurally generated archipelago map layout had been tested with the dice mechanic to further test the flow of the game (see appendix \ref{fig:gameflow}). However, it was the earliest prototype using digital components in its \textit{Material}, to generate text based events. 
...
A new design method for generating was then imagined. The technical details of how this methods are explained in the technical section of the report (see section \ref{sec:eventGeneration}). The design logic behind this system is determined as follows.
A \textit{Basic Outcome} is defined when performing the event. This outcome is chosen among several ones according to the following parameters:
\begin{enumerate}
\item The \textit{Location Type} chosen among several ones according to the \textit{Event Type}.
\item The \textit{Faction} controlling the island and the relation of that faction with the players (i.e. allied, neutral or enemy).
\item The \textit{Option Class} selected by the players.
\end{enumerate}
To generate the \textit{Final Outcome}, another set of parameters is taken into account:
\begin{enumerate}
\item The \textit{Success Rate} is calculated from a base, which is incremented or decreased according to the \textit{Faction} and the resources (\textit{Conditions}) that the players decide to use during the events.
\item Finally the \textit{Condition} and the \textit{Success Rate} determine the amount of resource that will be added to or removed from the \textit{Basic Outcome}, in order to generate the \textit{Final Outcome}.
\end{enumerate}
The \textit{Final Outcome} is the reward that the players get from the event. 
