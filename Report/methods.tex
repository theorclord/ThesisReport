...
Based on the principles previously stated and on the related design theory, a first game concept had to be found. The first design pattern that was considered was extracted from PCG-based games was the generation of \textit{nodes} that the players would need to travel to in order to reach a final destination. Examples of such mechanic can be found in \textit{FTL: Faster Than Light} \cite{game:ftl} or \textit{Out There} (Mi-Clos Studio, 2014)\cite{game:outthere}, which both inspired Archipelago in different. This pattern was called \textit{Events}. The players would have to 
