...
\begin{abstract}
A new set of games are emerging as a combination of digital and analogue games. These hybrid games tries to make use of digital components by the use of tablets, smartphones, and sometimes computers in collaboration with physical analogue board games. 
This thesis seeks to explore the possibilities of hybrid games through the design of one such game and implementing features which is not possible with only the analogue counterpart.
...
The game was designed over several iterations with feedback gathered from each prototype, both within the team and in some cases from external play tests. 
...
The game manages to produce features which cannot be easily reproduced in a non digital context and which definitely would be cumbersome to manage in that situation. The game shows that there is potential in the use of digital components to solve complex calculations and that it can serve to add features which would be to intrusive in a non digital setting.

%The world of games is growing rapidly, especially the digital game world which is putting more pressure on the analogue games. However, examples have shown that the digital game market does not cannibalize the tabletop game market, and interconnections exist between the two. As an attempt to exploit these two worlds at the same time and bring another audience to gather and play together, the hybrid games emerge giving a new design space to explore for designers by using smartphone and tablet devices to enhance traditional tabletop game play. Along with this is the problem for game makers to adapt to the parallel development of both a design application and the physical part of the game. This paper establishes a framework to understand the hybrid game genre in all its complexity. To do so, it is based on the creation  of a hybrid game using Procedural Content Generation not only to create a large amount of varied content, but also to support it mechanics, make both analogue and digital components irreplaceable and thus validating its hybrid denomination. Procedural Content Generation allows for game content to be generated at run-time, and removes the need for some physical elements such as cards, dice and reference-sheets and other aspects that are normally contained within the traditional tabletop games. As tabletop games usually require a rule-set that explains how certain actions happen, it is hoped that using Procedural Content Generation in a hybrid game, will shed light upon areas where board games and digital games can come together and shine as hybrid. The design and development process explained along the lines of this paper, as well as the data collected when prototyping and testing will help understanding the design space to explore for designers, and the technical challenges to solve in order to create such a game. 
